...
if (!fullFunnelApiKey && !legacyApiKey) {
  console.error('Error: Neither FULLFUNNEL_API_KEY nor API_KEY environment variable is set.');
...
console.log(`API_KEY: ${legacyApiKey ? '✓ Set' : '✗ Not set'}`);
console.log(`OPENAI_API_KEY: ${openaiApiKey ? '✓ Set' : '✗ Not set (only used for OpenAI calls)'}\n`);
...
async function runTests() {
  ...
  // Test 1: Using FULLFUNNEL_API_KEY
  try {
    console.log('Test 1: Request with FULLFUNNEL_API_KEY');
  ...
  // Test 7: Using OPENAI_API_KEY
  if (openaiApiKey) {
    console.log('\n-----------------------------------\n');
    try {
      console.log('Test 7: Request with OPENAI_API_KEY');
      const response = await axios.post(API_URL, payload, {
        headers: {
          'Content-Type': 'application/json',
